...
commands_wq = {"!wqscan": "Scan active WQs for an item or list of items.  e.g. *!wqscan Dinobone Charm, spellstaff, Wristwraps*",
...
commands_pronouns = {"!pronouns add": "Allows you to add a set of pronouns to your roles in Discord.", "!pronouns remove": "Allows you to remove a set of pronouns from your roles in Discord."}

commands_all = {"General": commands_general, "World Quests": commands_wq, "Pronouns": commands_pronouns}
...
async def commandHandler(message):
    ...
    if message.content.startswith(cmd):
        ...
        results = WQSearch.searchWQs(items=items)
    ...
    if message.content.startswith(cmd):
        ...
        try:
            with open(path.join(watchlist_filepath, message.author.name + '.json'), 'x') as f:
                json.dump(watchlist, f)

            await message.channel.send("{}'s watchlist saved.".format(message.author.mention))

        except FileExistsError:
            with open(path.join(watchlist_filepath, message.author.name + '.json'), 'r') as f:
                old_watchlist = json.load(f)
                watchlist['items'] = old_watchlist['items'] + watchlist['items']

            with open(path.join(watchlist_filepath, message.author.name + '.json'), 'w') as f:
                json.dump(watchlist, f)

            await message.channel.send("{}'s watchlist updated.".format(message.author.mention))
    ...
    if message.content.strip() == cmd:
        try:
            with open(path.join(watchlist_filepath, message.author.name + '.json'), 'r') as f:
                ...
                await message.channel.send("{}'s watchlist items: {}".format(message.author.mention, watchlist_items))
        except OSError:
            await message.channel.send("{} has no saved watchlist items.".format(message.author.mention))
    ...
    if message.content.startswith(cmd):
        ...
        try:
            ...
            await message.channel.send("Item(s) removed from {}'s watchlist.".format(message.author.mention))
    ...
    if message.content.strip() == cmd:
        try:
            ...
            await message.channel.send("{}'s watchlist has been cleared.".format(message.author.mention))
        except OSError:
            await message.channel.send("{} has no saved watchlist items.".format(message.author.mention))
    ...
    if message.content.strip() == cmd:
        ...
        await message.channel.send("{} - Type a number corresponding to the pronouns you want to add from the list below.\n{}".format(message.author.mention, pronoun_list))
        _waitForNonCommand(message.author.name, f_pronouns_add)  # add name to flag list for pending command
        client.loop.create_task(_expireFlag(f_pronouns_add, message.author.name, seconds=60))  # start a task to expire flag

    cmd = "!pronouns remove"
    if message.content.strip() == cmd:
        user_pronouns = [role for role in message.author.roles if (role.name in pronouns)] # get list of user's current pronouns
        pronoun_list = ""
        for i, pn in enumerate(user_pronouns):
            pronoun_list += "**{}. {}**\n".format(i+1, pn)

        await message.channel.send("{} - Type a number corresponding to the pronouns you want to remove from the list below.\n{}".format(message.author.mention, pronoun_list))
        _waitForNonCommand(message.author.name, f_pronouns_remove)  # add name to flag list for pending command
        client.loop.create_task(_expireFlag(f_pronouns_remove, message.author.name, seconds=60))  # start a task to expire flag
    ...
    if (message.author.name in f_pronouns_add):
        try:
            ...
            if input_num in range(len(pronouns)):
                ...
                pronoun_role = discord.utils.get(message.guild.roles, name=pronouns[input_num])  # find role ID
            ...
            await message.channel.send("{} - Please input a number between {} and {}.".format(message.author.mention, 1, len(pronouns)))
        except AttributeError:
            admin = discord.utils.get(message.guild.members, nick="Tenxian")
            await message.channel.send("{} - Something went wrong. {} needs to fix it.".format(message.author.mention, admin.mention))

    if (message.author.name in f_pronouns_remove):
        user_pronouns = [role.name for role in message.author.roles if (role.name in pronouns)] # get list of user's current pronouns
        try:
            input_num = int(message.content.strip()) - 1
            if input_num in range(len(user_pronouns)):

                pronoun_role = discord.utils.get(message.guild.roles, name=user_pronouns[input_num])  # find role ID
            ...
            await message.channel.send("{} - Please input a number between {} and {}.".format(message.author.mention, 1, len(user_pronouns)))
        except AttributeError:
            admin = discord.utils.get(message.guild.members, nick="Tenxian")
            await message.channel.send("{} - Something went wrong. {} needs to fix it.".format(message.author.mention, admin.mention))
...
async def checkActiveWQs(interval=30):
    ...
    while True:
        ...
        bot_channel = discord.utils.get(client.get_all_channels(), name=bot_watchlist_channel)

        watchlist_files = [f for f in listdir(watchlist_filepath) if path.isfile(path.join(watchlist_filepath, f))]

        for watchlist_filename in watchlist_files:
            with open(path.join(watchlist_filepath, watchlist_filename), 'r') as f:
                watchlist = json.load(f)

            results = WQSearch.searchWQs(items=watchlist['items'])

            results_msg = ""
            if (results):  # non-empty
                results_msg += "Active WQ item reward in {}'s watchlist!".format(watchlist['usermention']) + '\n'
...
def _getCommandList():
    ...
    for category in commands_all:
        ...
        for command in commands_all[category]:
            reply += "{}".format(command) + "  -  " + commands_all[category][command] + "\n"  # command - description
